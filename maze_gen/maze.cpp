...
Maze::Maze() {
	...
	TTF_Init();
	if(TTF_Init() == 1){
		cout << "BRUH: "<< TTF_GetError() << "\n";
	}else{
		cout << "pog\n";
	}
	
	SDL_CreateWindowAndRenderer(1500, 1200, 0, &window, &renderer);
	//window = SDL_CreateWindow("SDL_ttf in SDL2", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1500, 1200, 0);
	...
	string test = "ajksndfasjifbshdfbjhsdbfhjsdbfjhb";
	...
	Sizebutton_builder();	
								...
								//makes the graph size button(blue) and exit button(red)
